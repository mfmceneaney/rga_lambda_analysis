...
for base_dir, ch_sgasym_label, ch in zip(base_dirs,ch_sgasym_labels,chs):
    ...
    for binscheme_idx, binscheme_name in enumerate(binschemes.keys()):
        ...
        plot_results_kwargs_base['hist_paths'] = [
            os.path.abspath(os.path.join(RGA_LAMBDA_ANALYSIS,f'jobs/saga/test_getBinKinematicsTH1Ds__mc_rga__{ch}/out_1d_bins_binscheme_kinematics.root')),
            os.path.abspath(os.path.join(RGA_LAMBDA_ANALYSIS,f'jobs/saga/test_getBinKinematicsTH1Ds__dt_rga__{ch}/out_1d_bins_binscheme_kinematics.root')),
        ]
