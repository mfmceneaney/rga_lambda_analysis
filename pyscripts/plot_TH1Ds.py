...
parser = argparse.ArgumentParser(description='Script to plot 1D kinematic distributions from `getBinKinematicsTH1Ds` jobs on RGA data for the `Lambda -> proton pion` channel')
        ...
        for binvar in binvars:
            ...
            csv_path = os.path.abspath(
                os.path.join(
                    ...
                    f'jobs/saga/test_getBinKinematics__{rg}__{ch}/out_{rg}_1d_bins_{binvar}_kinematics.csv'
                )
            )
            hist_path = os.path.abspath(
                os.path.join(
                    RGA_LAMBDA_ANALYSIS_HOME,
                    f'jobs/saga/test_getBinKinematicsTH1Ds__{rg}__{ch}/out_{rg}_1d_bins_{binvar}_kinematics.root'
                )
            )
            ...
            for kinvar_x in kinvars:
                ...
                outpath = f'plot_TH1Ds__{rg}__{ch}__1d_{kinvar_x}__1d_{binvar}.pdf'
